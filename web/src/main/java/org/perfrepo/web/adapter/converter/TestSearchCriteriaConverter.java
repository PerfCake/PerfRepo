...
public class TestSearchCriteriaConverter {
    ...
    public org.perfrepo.web.service.search.TestSearchCriteria convertFromDtoToEntity(org.perfrepo.dto.test.TestSearchCriteria criteria) {
        ...
        criteriaEntity.setGroups(criteria.getGroupsFilter().stream().map(groupString -> { Group group = new Group(); group.setName(groupString); return group; }).collect(Collectors.toSet()));
        //TODO: don't use sets in the DTO search criteria
        criteriaEntity.setUid(criteria.getUniqueIdsFilter().stream().findFirst().orElse(null));
        criteriaEntity.setName(criteria.getNamesFilter().stream().findFirst().orElse(null));
