...
public class CreateManualTestData {
   ...
   Test createJDGCSTest() {
      ...
      test.groupId(testUserRole).uid("jdg_cs_test3").description("JDG Client/server test");
      test.metric("jdg_cs_avg_read_resp_time3", "0", "Average Read response time");
      test.metric("jdg_cs_avg_write_resp_time3", "1", "Average Write response time");
      test.metric("jdg_cs_requests_per_sec3", "2", "Number of requests per second");
      return test.build();
   }
